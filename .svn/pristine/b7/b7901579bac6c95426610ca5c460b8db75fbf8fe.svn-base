...
function generateReceipt(oSelect, sessId, patId, oDivSum, wait, canceled) {
	...
	var sParams = "&cli=" + cli + "&pat=" + patId + "&sess=" + sessId + "&usr=" + uid + "&ps=" + payString + "&income=" + iIncome;
	//alert(sParams);
	...
	if(valor.length > 0 && valor.indexOf("*") > -1) {
        ...
        if(aValue[0] == "OK") {
			document.location.replace("getPayment.php?cli=" + cli + "&pat=" + patId + "&UPD=" + sessId + "&rec=" + aValue[1] + "&wait=" + wait + "&canceled=" + canceled);
