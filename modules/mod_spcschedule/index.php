...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<? //if($browser == "FF") { ?>
        ...
        <script type="text/javascript" src="js/mod_spcschedule.js" ></script>
        <link rel='stylesheet' type='text/css' href="css/mod_spcschedule.css" />
        <script language="javascript" type="text/javascript">
	...
	<!--[CDATA[
        ...
        $j(function() {
            $j( "#date_ini" ).datepicker({ dateFormat: 'yy-mm-dd',showOtherMonths: true,
    			selectOtherMonths: true });
        });
        $j(function() {
            $j( "#date_end" ).datepicker({ dateFormat: 'yy-mm-dd',showOtherMonths: true,
    			selectOtherMonths: true });
        });
        $j(function() {
            $j( "#date_ini2" ).datepicker({ dateFormat: 'yy-mm-dd',showOtherMonths: true,
    			selectOtherMonths: true });
        });
        $j(function() {
            $j( "#date_end2" ).datepicker({ dateFormat: 'yy-mm-dd',showOtherMonths: true,
    			selectOtherMonths: true });
        });
		$j(function(){
			$j("#quincena_date_ini").datepicker({ dateFormat: 'yy-mm-dd',showOtherMonths: true,
				selectOtherMonths: true });
		});
		$j(function(){
			$j("#quincena_date_ini2").datepicker({ dateFormat: 'yy-mm-dd',showOtherMonths: true,
				selectOtherMonths: true });
		});
 ...
 //       document.oncontextmenu = function(e) {
  ...
  <body style="background:#FFFFFF;">
	...
	<tr>
		...
		<tr>
			<td><label class="title2">M&oacute;dulo de Horarios de Especialistas (<?=utf8_encode($nombreCli); ?>)</label></td>
...
<!-- Inicio del calendario -->                         
			...
			<table id="optionsTable" width="450" cellspacing="0" cellpadding="0">
				...
				<tr>
					...
					<td colspan="3">
						...
						<input type="radio" name="schType" id="2" onclick="openOptions(this.id)"/> Periodo
						<input type="radio" name="schType" id="3" onclick="openOptions(this.id)"/> Inactivo
						<input type="radio" name="schType" id="4" onclick="openOptions(this.id)"/> Quincenal
					</td>
				</tr>
				...
				<tr id="specialOptions6" style="display:none;">
					<td class="title">Fecha de Inicio:</td><td><input type="text" style="width:100px" name="quincena_date_ini" id="quincena_date_ini"/></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td colspan="3"><span id="drSelect"><select name="dr" id="dr"><option value="">---</option></select></span></td>
				</tr>
			</table>
		</form>
	</div>
	<div id="event_edit_container2" style="display: none;">
		<form>
			...
			<table id="optionsTable" width="450" cellspacing="0" cellpadding="0">
				...
				<tr>
					<td colspan="1" class="title">Tipo de Horario:</td>
					<td colspan="3">
						<input type="radio" name="schType2" id="1" onclick="openOptions(this.id)" checked="checked"/> Fijo 
						<input type="radio" name="schType2" id="2" onclick="openOptions(this.id)"/> Periodo
						<input type="radio" name="schType2" id="3" onclick="openOptions(this.id)"/> Inactividad
						<input type="radio" name="schType2" id="4" onclick="openOptions(this.id)"/> Quincenal
					</td>
				</tr>
				...
				<tr id="specialOptions5" style="display:none;">
					<td class="title">Fecha de Inicio:</td><td><input type="text" style="width:100px" name="quincena_date_ini2" id="quincena_date_ini2"/></td>
				</tr>
				<tr>
					<td colspan="1" class="title">Horario Inicial:</td>
					<td colspan="3"><select name="start2"><option value="">Select Start Time</option></select></td>
				</tr>
				<tr>
					<td colspan="1" class="title">Horario Final:</td>
					<td colspan="3"><select name="end2"><option value="">Select End Time</option></select></td>
				</tr>
				<tr>
					<td colspan="1" class="title">Especialidad:</td>
					<td colspan="3"><?=$spcSelect2;?></td>
				</tr>
				<tr>
					<td colspan="1" class="title">Doctor:</td>
					<td colspan="3"><span id="drSelect"><select name="dr2" id="dr2"><option value="">---</option></select></span></td>
				</tr>
			</table>
		</form>
		<div id="confirmEdit" style="text-align:center; display:none;">
				...
				<p>para editar el horario de este sill&oacute;n s&oacute;lo en la fecha seleccionada haga clic en "Editar".</p>
    	  </div>
	</div>
